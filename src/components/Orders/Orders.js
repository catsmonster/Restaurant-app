...
const Orders = ({tempTables, clickedTable, requestChange, setRequestChange}) => {
    let count = {};
    ...
    const menuArray = tempMenu.map((item, i) => {
        return <Menu key={tempMenu[i].id} id={tempMenu[i].id} name={tempMenu[i].name} price={tempMenu[i].price}
