...
const CategoriesWithMenu = ({
  ...
  const onClickMenu = ({ name, price }) => {
    if (path.includes('order_')) {
      ...
      updatedTempTables[clickedTable].orders.push({
        name: name,
        status: 'waiting',
        time: new Date(),
        table: clickedTable
      ...
      const indexOfSelectedItem = tempMenu.findIndex(
        item => item.name === name
      );
  ...
  const menuArrayActive = menuOfSelectedCategoryActive.map((item, i) => {
    return (
      <Menu
        key={menuOfSelectedCategoryActive[i].id}
        id={menuOfSelectedCategoryActive[i].id}
        name={menuOfSelectedCategoryActive[i].name}
        price={menuOfSelectedCategoryActive[i].price}
  ...
  const menuArrayHidden = menuOfSelectedCategoryHidden.map((item, i) => {
    return (
      <Menu
        key={menuOfSelectedCategoryHidden[i].id}
        id={menuOfSelectedCategoryHidden[i].id}
        name={menuOfSelectedCategoryHidden[i].name}
        price={menuOfSelectedCategoryHidden[i].price}
        onClickMenu={onClickMenu}
      />
    );
  });
  ...
  return (
    <div>
      <div className="menuArray">
        <Scroll>{categoriesArray}</Scroll>
        <Scroll>{menuArrayActive}</Scroll>
      </div>
      {path === 'customize' && hiddenCategoriesArray.length > 1 ? (
        <div>
          <h3>Click a menu item to bring it back from the trash</h3>
          <div className="menuArray">
            <Scroll>{hiddenCategoriesArray}</Scroll>
            <Scroll>{menuArrayHidden}</Scroll>
          </div>
        </div>
      ) : (
        <span></span>
