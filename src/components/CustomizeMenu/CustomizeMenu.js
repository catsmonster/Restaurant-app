...
const CustomizeMenu = ({
  ...
  const updateMenu = () => {
    ...
    if (
      menuInput.name.length > 0 &&
      !isNaN(Number(menuInput.price)) &&
      menuInput.category.length > 0
    ) {
      if (existingItems.includes(menuInput.name.toLowerCase())) {
        return alert(
          `The dish ${menuInput.name} already exists, please enter a unique dish name`
        );
      }
      tempMenu.push({
        id: lastKnownId + 1,
    ...
    <div className="statisticsMain">
      ...
      {addNewItemtoMenu ? (
        <div className="addDish">
          ...
          />
          <button className="addTable" onClick={() => updateMenu()}>
            Add dish
          </button>
          <h3>
            Click a menu item to remove it from the menu - This will not affect
            history
          </h3>
          <CategoriesWithMenu
        ...
        <div>
          ...
          <button
            className="addTable"
            onClick={() => setAddNewItemtoMenu(true)}
          >
            Add a new menu item
          </button>
        </div>
